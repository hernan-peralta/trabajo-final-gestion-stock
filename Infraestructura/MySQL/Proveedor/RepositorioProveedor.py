class RepositorioProveedor:
    ...
    def guardar(self, proveedor):
        ...
        datos_proveedor = (
        proveedor.nombre, proveedor.direccion_comercial, proveedor.id_localidad, proveedor.telefono, proveedor.email,
    ...
    def buscar_por_nombre(self, nombre_proveedor):
        self.cursor.execute(
            "SELECT proveedores.id, proveedores.nombre, proveedores.direccion_comercial, l.nombre, proveedores.telefono, proveedores.email, proveedores.dni, proveedores.cbu, proveedores.banco_cbu, proveedores.cuit, c_i.nombre, proveedores.observaciones FROM proveedores JOIN localidades AS l ON proveedores.id_localidad = l.id JOIN categoria_iva AS c_i on proveedores.id_categoria_iva = c_i.id WHERE proveedores.nombre LIKE %s",
            (nombre_proveedor.title(),))
        return self.cursor.fetchall()

    def buscar_por_localidad(self, localidad_proveedor):
        self.cursor.execute("SELECT * FROM proveedores WHERE localidad LIKE %s", (localidad_proveedor,))
        return self.cursor.fetchall()

    def buscar_por_provincia(self, provincia_proveedor):
        self.cursor.execute("SELECT * FROM proveedores WHERE provincia LIKE %s", (provincia_proveedor,))
        return self.cursor.fetchall()
